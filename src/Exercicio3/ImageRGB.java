...
public class ImageRGB {
  ...
  // (o f.bmp si falta la extensi�n en el nombre)
	...
	try{
	  ...
	  inBMP.skipBytes(28); // salta los datos no �tiles de la cabecera
	  // crea y rellena img
  ...
  // efecto: indica si se ha guardado el archivo
    ...
    try{
      ...
      int tamanoRelleno =(4-((ancho*3) % 4))%4; // n�mero de bytes de relleno al final de cada l�nea
      escribeEntero32bits(outBMP,altura*(ancho*3+tamanoRelleno)+54); // Tama�o del fichero
      ...
      outBMP.write(24); outBMP.write(0); // N�mero de bits por pixel
      escribeEntero32bits(outBMP,0); // sin compresi�n
      escribeEntero32bits(outBMP,altura*(ancho*3+tamanoRelleno)); // tama�o de la imagen en bytes
      escribeEntero32bits(outBMP,2851); escribeEntero32bits(outBMP,2851); // resoluci�n en ancho x alto
  ...
  // efecto: escribe n en la salida sobre 4 bytes seg�n el formato littel indian
    try {
      salida.write((n) & 0xFF);
  ...
  // resultado : el pixel en posici�n (i,j)
  ...
  // requisito : 0<=i<this.altura() et 0<=j<this.ancho()
  // efecto : cambia por pix el pixel en posici�n (i,j)
