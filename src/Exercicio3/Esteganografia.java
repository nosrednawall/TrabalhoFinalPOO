...
public class Esteganografia {
	...
	static final String caracteres = "abcdefghijklmnopqrstuvwxyzŕâéčęëîďôůűü˙ç"
			+ "ABCDEFGHIJKLMNOPQRSTUVWXYZŔÂČÉĘËÎĎÔŮŰÜÇ" + "0123456789" + " ,.;:!?'Ťť\"" + "()+-*/=<>[]{}|%" + "&~#@\\^"
			+ "\t\n\r" + "§"; // <- Caracter final
	...
	static PixelRGB escondeCaracter(PixelRGB pixel, char c) {
		...
		// Sustituimos el bit menos significativo de cada pixel por las cifras
		// creadas
		int rr = (r / 5) * 5 + cr;
		int gg = (g / 5) * 5 + cg;
		int bb = (b / 5) * 5 + cb;
